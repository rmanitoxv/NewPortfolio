...
import resume from "../files/Russel Manito.pdf"
	...
	return (
		<div className="bg-linear-to-b from-neutral-950 to-neutral-900 text-white min-h-screen flex justify-center items-center">
			<div className="flex flex-col justify-center h-full lg:w-3/5 items-center">
				...
				<div
					className={`flex flex-wrap justify-center gap-2 transition-all duration-1000 delay-300 ${
						isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
					}`}
				>
					{langs.map((lang, index) => {
						return (
							<div
								key={lang}
								className={`border border-blue-400/30 rounded-full px-2 2xl:px-4 py-1 hover:text-neutral-950 hover:bg-white hover:-translate-y-1 cursor-default transition-all 2xl:text-xl xl:text-sm transition-all duration-300 delay-${
									(index * 50) % 500
								}`}
							>
								{lang}
				...
				<div
					className={`flex flex-col items-center w-full mt-8 transition-all duration-1000 delay-500 ${
						isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
					}`}
				>
					...
					<div className="flex gap-4 justify-center items-center text-3xl">
						<a
							href="https://www.linkedin.com/in/rrmanitoxv/"
							className="hover:text-blue-500 transition-colors"
						>
							<FaLinkedin />
						</a>
						<a
							href="https://github.com/rmanitoxv"
